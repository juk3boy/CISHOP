...
class Cart extends MY_Controller
{
    ...
    public function add()
    {
        ...
        if (!$_POST || $this->input->post('qty') < 1) {
            ...
            /** setelah kita mengoverride pada tabel yg ada di model */
            $product            = $this->cart->where('id', $input->id_product)->first();

            $subtotal           = $product->price * $input->qty;

            $this->cart->table  = 'cart';
            $cart               = $this->cart->where('id_user', $input->id)->where('id_product', $input->id_product)->first();
            ...
            if ($cart) {
                ...
                if ($this->cart->where('id', $cart->id)->update($data)) {
                    ...
                    $this->session->set_flashdata('erorr', 'Oops !!! Terjadi kesalahan');
                }
            ...
            $data = [
                ...
                'qty'           => $input->qty,  // /** disini menumlahkan */
                'subtotal'      => $subtotal
            ];
    ...
    public function update($id)
    {
        ...
        $subtotal           = $data['input']->qty * $product->price; //$product->price diambil dari atasnya
        ...
        // kemudian kita lakukan overide lagi menjadi cart
        $this->cart->table = 'cart';
